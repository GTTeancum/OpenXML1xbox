...
static ULONGLONG test_release_tick[4];
static void test_file_input(uint32_t frame) {
    if(!test_input_path[0]) return;
    for(unsigned port=0;port<4;++port) if(test_release_tick[port] && GetTickCount64()>=test_release_tick[port]) {
        XBOX_GAMEPAD state={0}; xml1_input_test_state(port,1,&state);
        test_release_tick[port]=0;
        fprintf(stderr,"[INPUT FILE] frame=%u released port=%u\n",frame,port);
    }
    ...
    /* p1:..p4: address physical test slots, before game-port routing. Legacy
       commands retain slot zero. No host input is generated by this harness. */
    unsigned port=0;const char *command=action;
    if(action[0]=='p' && action[1]>='1' && action[1]<='4' && action[2]==':') {
        port=(unsigned)(action[1]-'1');command=action+3;
    }
    char parts[32]; strcpy(parts,command);
    char *part=parts;
    for(;;) {
    ...
    test_input_id=id; test_release_tick[port]=duration?GetTickCount64()+duration:0;
    xml1_input_test_state(port,strcmp(command,"disconnect")!=0,&state);
    fprintf(stderr,"[INPUT FILE] id=%u frame=%u action=%s duration_ms=%u port=%u (process-local only)\n",id,frame,action,duration,port);
}
...
static DWORD physical_poll(int slot,XBOX_INPUT_STATE *state)
{
    if(slot<0 || slot>=4)return DISCONNECTED;
    if(test_mode) {
...
static DWORD poll(unsigned port, XBOX_INPUT_STATE *state, int consume)
{
    ...
    if(!xml1_pc_channel_read(&input,1)) {
        ReleaseSRWLockExclusive(&pc_poll_lock);return physical_poll((int)port,state);
    }
    int slot=xml1_pc_controller_slot(&input.settings,port);
    DWORD result=physical_poll(slot,state);
    xml1_pc_channel_controller_connection(port,!result);
    if(result)memset(state,0,sizeof(*state));
    /* Track physical activity before keyboard merging. Enumeration must not
       consume transitions, and idle sticks must not steal keyboard prompts. */
    static XBOX_GAMEPAD last_physical[4];
    if(consume) {
        Xml1PcGamepad capture_pad;
        memcpy(&capture_pad,&state->Gamepad,sizeof(capture_pad));
        xml1_pc_channel_controller_state(port,!result,
            xml1_pc_controller_sources(&capture_pad,0),xml1_pc_controller_sources(&capture_pad,1));
        const XBOX_GAMEPAD *physical=&state->Gamepad;
        int active=physical->wButtons!=0;
        for(unsigned i=0;i<8;++i)active|=physical->bAnalogButtons[i]>30;
        active|=abs((int)physical->sThumbLX)>7849 || abs((int)physical->sThumbLY)>7849 ||
                abs((int)physical->sThumbRX)>8689 || abs((int)physical->sThumbRY)>8689;
        if(!result && input.controls.focused && active &&
           memcmp(physical,&last_physical[port],sizeof(*physical)))
            xml1_pc_channel_controller_active();
        last_physical[port]=*physical;
    }
    if(!result) {
        Xml1PcGamepad physical;
        memcpy(&physical,&state->Gamepad,sizeof(physical));
        xml1_pc_remap_controller(&input.settings,port,input.native_menu,&physical);
        memcpy(&state->Gamepad,&physical,sizeof(physical));
    }
    ...
    /* Connection probes can see a transition first. Log what the game actually
       consumes independently so a peek cannot hide a mouse or key event. */
    static XBOX_GAMEPAD last_consumed[4];
    if(pc_test_mode && consume && memcmp(&state->Gamepad,&last_consumed[port],sizeof(state->Gamepad))) {
        last_consumed[port]=state->Gamepad;
        fprintf(stderr,"[PC GAME INPUT] port=%u packet=%u buttons=%04X A=%u B=%u LX=%d LY=%d RX=%d RY=%d\n",
...
void xml1_XInitDevices(void)
{
    ...
    test_input_path[0]=0; test_input_id=0; memset(test_release_tick,0,sizeof(test_release_tick));
    ...
    previous_mask = 0;
    if (!test_mode && !pc_test_mode) xbox_InputInit();
...
void xml1_XInputSetState(void)
{
    ...
    if (!result && !test_mode && !pc_test_mode) {

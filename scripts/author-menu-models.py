...
def main():
    ...
    declared_parts = Path(layout + args.input.suffix).parts
    output_parts = args.output.resolve().parts
    if tuple(part.lower() for part in output_parts[-len(declared_parts):]) != tuple(
            part.lower() for part in declared_parts):
        raise ValueError(f'Output must retain the PKGB-declared relative path: '
                         f'{layout}{args.input.suffix}')
    bindings = {}
    skipped = []

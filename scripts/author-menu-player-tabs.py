"""Populate the copied XML2 menu IGB with four player-profile selectors.
...
assert not any(name.startswith('pc_player') for name in transforms)
added=[]
def anchor(name,x,z):
...
for player in range(4):
 anchor('pc_player'+str(player+1),256+58*player,305)
 name='pc_profile_focus_'+str(player+1)
 anchor(name,272+58*player,305)
 # Reuse the package's recolored native selection geometry, sized to one tab.
 node=transforms[name];matrix=list(field(node,8))
 matrix[0]=.36;matrix[5]=1.;matrix[10]=.40;matrix[13]=-119.
 put(node,8,tuple(matrix))
# Keep the existing XML1 branding above the profile selectors.
node=transforms['label_controls'];matrix=list(field(node,8));matrix[14]=321;put(node,8,tuple(matrix))
items={i.get('name'):i for i in menu.getroot().findall('item')}
for player in range(1,5):
 name='pc_player'+str(player)
 ET.SubElement(menu.getroot(),'item',name=name,style='STYLE_SMALL',animtext_scene='options',
  gamevar='pcnative_profile_'+str(player),text='Player '+str(player),
  usecmd='pcnative_profile_'+str(player),textalignx='TEXT_ALIGN_LEFT',
  up='pc_player'+str(player-1) if player>1 else 'label_accept',
  down='pc_player'+str(player+1) if player<4 else 'label_effects_volume',
  left='pc_player'+str(player-1 if player>1 else 4),
  right='pc_player'+str(player+1 if player<4 else 1),
  focusmodel=focus_model,focusitemname='pc_profile_focus_'+str(player))
 ET.SubElement(menu.getroot(),'item',name='pc_profile_focus_'+str(player),type='MENU_ITEM_MODEL',enabled='false')
items['label_effects_volume'].set('up','pc_player4');items['label_accept'].set('down','pc_player1')
lists=[]
...
output.with_suffix('.player-tabs.json').write_text(json.dumps({
...
print('Authored four native player selectors:',output)

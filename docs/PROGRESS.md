...
### 2026-09-13: SAR operand-width correction prepared during human test

Confirmed the previously observed narrow-SAR translation defect independently of the running game. Compiled lifted SAR AL,CL with input80/count1 returns40 before correction instead of signed C0. Patch27 xboxrecomp-sar-operand-width.patch sign-extends the actual8/16/32-bit operand before arithmetic shifting and masks the x86 count to five bits. Carry is extracted from the sign-extended source, preserving prior CF for masked-zero counts and handling counts beyond narrow operand width. Regression executes396 value/count combinations with STC/SAR/ADC to check both result and carry, including upper-register preservation and counts0/31/32/33/255. Passes after correction. All178 toolkit tests and16 subtests pass. Full27-patch reverse verification passes. This does not fix ROL/ROR width or general shift condition-flag snapshots; do not claim complete shift/rotate support. Upstream PR title search for shift/SAR/rotate found no match among100 returned PRs; local remote branches origin/main and origin/work/v0.7.0-non-local only (not a fresh remote branch inventory).

IMPORTANT: game generated C and executable have NOT been regenerated/rebuilt for patch27, nor for the latest ADX diagnostic expansion. Human playtest PID9260 / exec26412 is still using its original18:32:44UTC launched build. Preserve user control and await feedback. Next after that run: regenerate and build optimized game, validate SAR in title camera paths, trace actual CRI movie pan state, and continue normal level1 traversal/fidelity verification. Goal remains incomplete.
...
### 2026-09-13 19:05 UTC: human feedback and save enumeration ABI correction

Human playtest ended by user closing the window. User reports improved sound quality with remaining stutter, occasional FPS drops, and a completed save that did not appear afterward. The expected worker-pipe disconnect on window closure is not a gameplay crash. Original UDATA is preserved; backup work/udata-backup-20260913-1857 includes the 195608-byte save.dat and SaveMeta named "Game 1 00:14 - Central Park". Normal subway traversal is confirmed by the user; complete level1 and defect-free audio remain unproven.

Identified a concrete ordinal207 ABI error against local nxdk declarations, Cxbx reference, and the title's original call sites1A2A52/1A2DB5: NtQueryDirectoryFile has TEN arguments including FileInformationClass at index7. XboxRecomp omitted that argument, misread the filter and restart, and popped36 instead of40 argument bytes. Patch28 xboxrecomp-directory-query-abi.patch corrects both backend signatures, bridge arguments and cleanup, rejects unsupported information classes, and excludes host dot directories. Synthetic import regression failed before with ESP00020028 instead of0002002C, then passed filter, continuation, restart, dot exclusion, and unsupported-class cases. Full28-patch reverse validation passes. Game regeneration/build in progress, including pending SAR patch27 and expanded ADX diagnostics. Save listing/loading in the actual title NOT yet verified. Test root work/save-load-fixture holds copied saves and links only immutable asset directories to original game data.

User asks about a future15-20minute gameplay MP4 for YouTube. Discussed process-local DX8 frame plus audio recording with bounded encoder queues and synchronized timestamps; no recording harness implemented yet. Current priorities remain save listing/loading, audio stutter and frame drops before recording proof.

### 2026-09-13: save fix built, playback overhead removed, awaiting user

User explicitly requests that the game remain closed and that work stall after the reported issues and recording route are handled. No game launch was performed. Full optimized regeneration/build completed, incorporating SAR patch27, directory ABI patch28 and expanded ADX diagnostics. Directory synthetic-import regression passes both the local patch stack and a clean upstream-main worktree; upstream PR51 https://github.com/sp00nznet/xboxrecomp/pull/51 (commit9ffb180). Original save backup and isolated copy match SHA256 10BC10EAEB3F9AB6FCF93E5B99A7F644B1BF56511499FB0146B1F2280AE658F0. Actual game save-list and load verification remains pending the user's return.

Preserved human renderer log at work/human-143244-renderer.log. Six renderer commands exceeded50ms, maximum172ms; two occurred on periodic screenshot frames (37500=172ms and16440=78ms). This correlation does not establish every stall's cause. Cumulative PCM submission time lagged wall time by9.35sec over1165.35sec; worst2-second sample interval took3.978sec. It is evidence of audio scheduling gaps, not a recording of the precise audible glitch timeline. Do NOT claim stutter resolved by unit tests or raw PCM continuity.

Human playtest launcher now sets XML1_DX8_NO_CAPTURE=1; renderer honors it to omit periodic synchronous backbuffer/BMP captures. Patch29 makes per-volume-change VP logging opt-in with XML1_TRACE_VOICE_MIX (initial/nonzero voice diagnostics remain) and adds XAudio2 queue-empty observations and maximum inter-submission gaps. No buffer-depth or original DSP/mix semantics changed. Renderer and optimized executable rebuilt successfully; kernel-directory-test, apu-output-lock-test and apu-output-test pass, the audio tests explicitly open no audio device. Full29-patch reverse validation passes. Audio stutter/FPS drops remain unverified/open; do not describe these overhead reductions as full resolution.

Recording route documented in docs/RECORDING.md and copied to outputs/Recording-OpenXML1.md. Installed OBS32.1.1 confirmed via binary version; its tagged source supports D3D8 hooks. Recommend user-operated OBS Game Capture of xml1-dx8-worker, Window Capture fallback, separate audible output capture because XAudio2 belongs to xml1-boot-probe, H264/AAC and Hybrid MP4, preserved4:3. Short synchronization/performance check must precede15-20minute take. No OBS or desktop automation and no recorder implemented. Next required evidence is user-authorized game play; stop at this handoff without launching the game. Goal incomplete and not eligible for completion; first current verification-wait audit, not a repeated blocked threshold.

### 2026-09-13 23:00 UTC: fix repeated XAudio2 depletion behind reported crackle

User requested urgent resolution of crackle heard during the OBS smoke capture. That run's new queue telemetry showed774 empty-queue observations across11250 submissions/60 seconds of PCM; individual submission gaps reached124.814ms, with common gaps15-30ms. The backend allocated three1024-frame slots but received256-frame native DSP blocks, reducing its intended roughly64ms headroom to16ms. XAudio2 began playing before any PCM was queued.

Patch30 xboxrecomp-dsp-output-headroom.patch gives native DSP output twelve256-frame buffers (64ms capacity) and starts after six real blocks (32ms) are queued. It preserves the original stereo samples, original DSP and legacy3x1024 backend behavior; no synthetic interpolation, gain changes or dropped sample blocks. Invalid submissions are rejected rather than silently truncated. Start failures latch a device error without resubmitting an already accepted block.

Native queue regression compiles the real backend with mocked XAudio2: checks priming threshold, full-capacity backpressure,30 ring wraps without changing queued PCM, invalid submissions and deferred-start failure without duplication. Passes without opening an audio device. Existing upstream legacy XAudio2 test also passes. Optimized game rebuilt. Full30-patch reverse validation passed before cleanup of unused test code; test-only cleanup rebuilt/retested successfully.

...
static int attempts,accepted,waits;
int xa2_init(void) { abort();return 0; }
void xa2_shutdown(void) { abort(); }
...
int main(void) {
    ...
    if(attempts!=3||accepted!=1||waits!=2||memcmp(received,expected,sizeof(expected))) return 1;
    puts("PASS: complete DSP stereo block preserved and accepted once after a transient completion timeout; no audio device opened");
    return 0;
}
